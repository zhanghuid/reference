...
如果域名解析为多个地址，则所有这些地址都将以循环方式使用。另外，地址可以被指定为[服务器组](nginx/ngx_http_upstream_module.md)。

参数值可以包含变量。在这种情况下，如果地址被指定为域名，则在所描述的[服务器组](nginx/ngx_http_upstream_module.md)中搜索名称，如果未找到，则使用[解析器](nginx/ngx_http_core_module.md#resolver)来确定。
